"""Advanced Portfolio Analysis - Efficient Frontier, Backtesting, Drawdown, Monte Carlo"""
...
class AdvancedPortfolio:
    ...
    def efficient_frontier(self, points=100):
        ...
        min_vol_w = res.x
        min_vol_ret, min_vol_vol, _ = self.port_stats(min_vol_w)
        
        # Max return portfolio
        max_ret_w = np.zeros(n)
        max_ret_w[np.argmax(self.mean_returns.values)] = 1
        max_ret, max_ret_vol, _ = self.port_stats(max_ret_w)
    ...
    def monte_carlo(self, weights, years=5, sims=10000):
...
def create_advanced_portfolio_tab():
    st.markdown('<div class="section-header">🏦 Advanced Portfolio Analysis</div>', unsafe_allow_html=True)
    ...
    with col1:
        preset_name = st.selectbox("Preset Portfolio", list(presets.keys()), key="adv_preset")
    with col2:
        default_tickers = ",".join(presets[preset_name]) if presets[preset_name] else "AAPL, MSFT, NVDA, AMZN, GOOGL"
    ...
    if st.button("🚀 Run Analysis", type="primary", use_container_width=True):
        ...
        with st.spinner("Generating efficient frontier..."):
            frontier = ap.efficient_frontier(points=50)
        ...
        if frontier:
            ...
            st.caption(f"**Max Sharpe**: Return {rets[best_idx]:.1f}%, Vol {vols[best_idx]:.1f}%, Sharpe {sharpes[best_idx]:.2f}")
        ...
        fig.add_trace(go.Scatter(
            x=cum100.index, y=cum100.values, name='Max Sharpe Portfolio',
        ...
        # ===== 4. MONTE CARLO =====
        st.markdown("### 🎲 Monte Carlo Simulation (5 Years)")
        
        with st.spinner("Running 5,000 simulations..."):
            sims = ap.monte_carlo(w_ms, years=5, sims=5000)
        
        fig = go.Figure()
        percentiles = [5, 25, 50, 75, 95]
        colors = ['#ef4444', '#f59e0b', '#10b981', '#34d399', '#667eea']
        ...
        fig.update_layout(title='5,000 Simulated Portfolio Paths', xaxis_title='Trading Days',
        ...
        fig = go.Figure()
        fig.add_trace(go.Histogram(x=final_values, nbinsx=50, marker_color='#667eea',
                                   histnorm='probability'))
        fig.add_vline(x=100, line_dash="dash", line_color="#ef4444", annotation_text="Initial $100")
        fig.update_layout(title='Distribution of Final Portfolio Value',
                         xaxis_title='Portfolio Value ($)', yaxis_title='Probability',
                         template='plotly_white', height=300)
        st.plotly_chart(fig, use_container_width=True)
        
        prob_profit = (final_values > 100).mean() * 100
        prob_50pct = (final_values > 150).mean() * 100
        prob_loss20 = (final_values < 80).mean() * 100
        
        st.success(f"""
        **Monte Carlo Summary (5 Years):**
        - **{prob_profit:.0f}%** probability of positive returns
        - **{prob_50pct:.0f}%** chance of 50%+ returns
